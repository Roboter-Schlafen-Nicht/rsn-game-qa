...
class TestHextrisScoreParamForwarding:
    ...
    def test_accepts_score_region(self):
        """HextrisEnv accepts score_region and forwards to BaseGameEnv."""
        env = _make_env(
            reward_mode="score",
            score_region=(540, 402, 200, 80),
        )
        assert env._score_region == (540, 402, 200, 80)
        assert env._score_ocr is not None

    def test_accepts_score_ocr_interval(self):
        """HextrisEnv accepts score_ocr_interval and forwards to BaseGameEnv."""
        env = _make_env(
            reward_mode="score",
    ...
    def test_accepts_score_reward_coeff(self):
        """HextrisEnv accepts score_reward_coeff and forwards to BaseGameEnv."""
        env = _make_env(
            reward_mode="score",
