...
- [x] Logging pipeline
  - [x] Extend help loop to save:
    - `data/live_logs/frame_*.png` from `screencapnp`.
    - Matching `frame_*.json` with detections, `will_click`, `reason`, timestamps.
  - [x] Log only when HELP is visible or at a low fixed frequency to control disk usage.

- [x] Build `twin_dataset`
  - [x] Implement `build_twin_dataset.py`:
...
- [x] Train YOLO v2
  - [x] Merge manual dataset + `twin_dataset` into a new training config (Roboflow or local YAML).
  - [x] Train a new model (e.g. `runs/detect/train_help_v2/weights/best.pt`) on XPU.
  - [x] Validate on a held‑out manual set and a few manual screenshots.

- [x] Deploy v2 model
  - [x] Switch `HelpOnlyPolicy` to use v2 weights.
  - [x] Shadow‑run v1 vs v2 (log both, let v1 control) and verify v2:
...
- [x] Define Gym environment
  - [x] Implement `LastWarHelpEnv(gym.Env)`:
...
- [x] Reward shaping
  - [x] Reward +1.0 when clicking HELP makes HELP disappear (success).
  - [x] Penalize useless clicks (no HELP visible or HELP remains), e.g. −0.2/−0.3.
  - [x] Small negative for NOOP when HELP is visible; tiny negative otherwise.
  - [x] Small negative for BACK to avoid spam.

- [x] Train RL agent
  - [x] Plug env into an RL algorithm (e.g. PPO with MLP policy).
  - [x] Train for N episodes in shadow mode (optionally not sending taps).
  - [x] Inspect trajectories and rewards to ensure it learns sensible behavior.

- [x] Integrate RL policy
  - [x] Export the trained RL policy.
  ...
  - [x] Run limited live sessions and compare against v2 heuristic:
